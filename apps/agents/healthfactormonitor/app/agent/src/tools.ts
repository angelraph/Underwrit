...
import { getAccountLiquidity, getVUsdtBorrowBalance } from "./venus.js";
  ...
  venus_account_health: tool({
    ...
    inputSchema: z.object({
      ...
      safety_buffer_usd: z
        .number()
        .optional()
        .describe("USD headroom below which the account is graded AT_RISK (default 1)"),
    }),
    execute: async ({ address, safety_buffer_usd }) => {
      const account = address as Address;
      const liquidity = await getAccountLiquidity(account);
      const outstandingVUsdtBorrow = await getVUsdtBorrowBalance(account);
      return {
        account,
        tier: assessRisk(liquidity, safety_buffer_usd ?? 1),
        liquidityUsd1e18: liquidity.liquidityUsd.toString(),
        shortfallUsd1e18: liquidity.shortfallUsd.toString(),
        outstandingVUsdtBorrowRaw: outstandingVUsdtBorrow.toString(),
      };
    },
  }),
