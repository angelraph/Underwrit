...
import { z } from "zod";
import { getSupplyApyPct, VENUS_TESTNET, vBep20Abi, vBnbAbi } from "./venus.js";
...
export const LLM_READ_TOOLS: ToolSet = {
  ...
  // --- Venus Protocol (read-only — the actual routing decision lives in yieldRouterCore.ts, never here) ---
  venus_yield_comparison: tool({
    description:
      "Compare real, live Venus Protocol Core Pool supply APY across BNB and " +
      "USDT markets on BSC Testnet — annualized from an actually-measured " +
      "on-chain block time, not an assumed constant. Read-only.",
    inputSchema: z.object({}),
    execute: async () => {
      ...
      return {
        vBNB_supplyApyPct: Number(bnbApy.toFixed(3)),
        vUSDT_supplyApyPct: Number(usdtApy.toFixed(3)),
        best: bnbApy >= usdtApy ? "vBNB" : "vUSDT",
      };
    },
  }),
