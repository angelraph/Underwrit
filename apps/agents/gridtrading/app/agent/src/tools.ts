...
import { z } from "zod";
import { getPoolState } from "./pancake.js";
...
export const LLM_READ_TOOLS: ToolSet = {
  ...
  // --- PancakeSwap V3 (read-only — the actual grid decision lives in gridTradingCore.ts, never here) ---
  pancake_pool_state: tool({
    description:
      "Live PancakeSwap V3 WBNB/USDT (0.01% fee tier) pool state on BSC Testnet: " +
      "current tick and spot price. Read-only.",
    inputSchema: z.object({}),
    execute: async () => {
      const pool = await getPoolState();
      return { tick: pool.tick, sqrtPriceX96: pool.sqrtPriceX96.toString(), rawPrice: pool.rawPrice };
    },
  }),
