...
**Say:** "This agent has been continuously monitored for real, every six hours, for two weeks straight, not just the one day it happened to take a real action. Most of that time it correctly did nothing, because nothing needed doing, which is itself the honest signal: it's not reacting to noise."
...
**One extra note learned the hard way**: if the browser ever shows more than one saved passkey for this site (e.g. from testing on multiple devices), the OS picker labels them all identically and there's no way to tell which wallet each one is from the picker alone. Stick to one browser profile for the actual recording so this never comes up.
...
- **"Is the ERC-8183 / x402 bonus work real?"** Both are fully coded and deployed (a real 402-challenge-issuing sell endpoint, a real ERC-8183 buyer-hire function using the SDK's own testnet contracts), but the full paid round-trip isn't independently verified yet, it's blocked on acquiring testnet $U to actually pay with. Say so plainly if asked; don't claim it's proven when it isn't.
